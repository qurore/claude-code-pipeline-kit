 ...
 */
export function readManifest(manifestPath: string): PipelineManifest | null {
  try {
    const raw = fs.readFileSync(manifestPath, 'utf-8');
    const parsed = JSON.parse(raw) as PipelineManifest;
    if (parsed.stop_injections === undefined) parsed.stop_injections = 0;
    return parsed;
  } catch (err) {
 ...
 */
export function writeManifestAtomic(manifestPath: string, manifest: PipelineManifest): void {
...
function readLockSafely(lockPath: string): LockData | null {
  try {
 ...
 */
export function withLock<T>(lockPath: string, fn: () => T, ttlMs = LOCK_TTL_MS): T {
  ...
  while (true) {
    try {
      ...
      try {
        ...
        try { fs.unlinkSync(lockPath); } catch { /* lock already removed */ }
      }
    } catch (err) {
      const code = (err as NodeJS.ErrnoException).code;
...
export function mutateManifest(
  manifestPath: string,
  mutator: (m: PipelineManifest) => PipelineManifest | void,
): PipelineManifest {
  return withLock(`${manifestPath}.lock`, () => {
    const m = readManifest(manifestPath);
    if (!m) throw new Error(`Manifest not found: ${manifestPath}`);
    const next = (mutator(m) ?? m) as PipelineManifest;
