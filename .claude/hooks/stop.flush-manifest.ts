...
export async function main(now = Date.now()): Promise<number> {
  ...
  try {
    ...
    emitSignal({ hook: HOOK, decision: 'warn', reason: 'flush-failed', context: { error: (err as Error).message }, docsAnchor: 'stop-flush' });
  }

  const orphans = detectOrphans(active.dir, active.manifest);
  const warnings: { type: 'missing' | 'orphan'; path: string; suggestion?: string }[] = [];
  ...
  try {
    ...
    if (fs.existsSync(sweepDir)) {
      for (const f of fs.readdirSync(sweepDir)) {
        if (!f.startsWith('.trivial-fix-active.consumed.')) continue;
