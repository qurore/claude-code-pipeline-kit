...
export function findMissing(content: string): string[] {
  ...
  if (!hasAnyRequired) missing.push(`one of: ${REQUIRED_SECTIONS.join(' / ')}`);
  return missing;
}
...
export async function main(stdinJson: string): Promise<number> {
  ...
  if (!filePath || !/\.claude\/commands\/[^/]+\.md$/.test(filePath)) return 0;
