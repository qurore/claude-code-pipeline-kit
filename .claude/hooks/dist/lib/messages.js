export const MIGRATION_REMINDER = 'Migration file detected. Remember to apply this migration to all environments per your project\'s deployment documentation (e.g., dev, staging, production).';
...
export function resumePhaseCommand(pipeline, currentPhase) {
    if (pipeline === 'drw')
        return '/defect-fix';
    if (currentPhase.startsWith('eiw-stage') || currentPhase.startsWith('D'))
        return `/${currentPhase}`;
...
export function nextSkillCommand(pipeline, currentPhase) {
    if (pipeline === 'se' && SE_PHASE_SKILLS[currentPhase])
        return SE_PHASE_SKILLS[currentPhase];
    if (pipeline === 'drw')
        return '/defect-fix';
    if (pipeline === 'eiw' && currentPhase.startsWith('eiw-stage'))
        return `/${currentPhase}`;
    return resumePhaseCommand(pipeline, currentPhase);
}
export function formatResumeEntry(e, now = Date.now()) {
...
export const messages = {
    phaseGateBlocked: (skill, missingPhase) => `Phase gate: prior phase ${missingPhase} not approved. Run /${missingPhase} before /${skill}.`,
    outputGateBlocked: (filePath, currentPhase) => {
    ...
    mandatoryOpusBlocked: (subagentType, description, callerSkill, promptPreview) => {
    ...
    uiLintWarning: (violations) => violations.map((v) => `${v.file}:${v.line}: ${v.rule} — ${v.snippet}`).join('\n'),
    skillLintWarning: (file, missing) => `Skill lint (${file}): missing sections: ${missing.join(', ')}. Reference template: .claude/skills/agent-harness.md "Use canonical template".`,
    migrationReminder: () => MIGRATION_REMINDER,
    resumeBanner: (entries, now = Date.now()) => entries.map((e) => formatResumeEntry(e, now)).join('\n'),
    archivalSummary: (archived, staleCount) => {
        if (archived.length === 0 && staleCount === 0)
            return '';
        const parts = [];
        if (archived.length > 0)
            parts.push(`Archived ${archived.length} run(s) to .claude/pdca-archive/runs/.`);
        if (staleCount > 0)
            parts.push(`${staleCount} stale run(s) flagged (>30d inactive).`);
        return parts.join(' ');
    },
    orphanWarning: (orphans) => orphans
        ...
        .join('\n'),
    bypassWarning: (envVar) => `Warning: ${envVar} is set in this session. Hook enforcement is reduced. Unset to re-enable.`,
    pipelineStalled: (runId, currentPhase, max) => `Pipeline ${runId} stalled at phase ${currentPhase} after ${max} stop injections. Type \`/abort-pipeline\` to acknowledge, or re-invoke the pipeline to resume.`,
    stopBlockReason: (pipeline, runId, currentPhase, iteration, maxIterations, nextInjection, max, nextCommand) => [
    ...
    pipelineCompleted: (pipeline, runId, totalPhases, totalRestarts, totalStopInjections, durationMs) => {
