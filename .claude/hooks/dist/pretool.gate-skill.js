...
export function priorPhaseFor(skillName) {
    const m1 = skillName.match(/^\/se-([1-9])(?:-|$)/);
    if (m1) {
        const n = Number(m1[1]);
        if (n === 1)
            return '/se-0-codebase-exploration';
        return `/se-${n - 1}`;
    }
...
export async function main(stdinJson, cwd = process.cwd()) {
    ...
    const found = runs.some(({ manifest }) => manifest.phase_history.some((h) => h.status === 'approved' && skillPathMatchesPhase(prior, h.phase)));
    if (found) {
...
export function skillPathMatchesPhase(skillPath, phaseId) {
    const m1 = skillPath.match(/^\/se-([0-9]+)/);
    if (m1)
        return phaseId === m1[1];
    const m2 = skillPath.match(/^\/eiw-stage([0-9]+)/);
    if (m2)
        return phaseId === `eiw-stage${m2[1]}` || phaseId === m2[1];
    const m3 = skillPath.match(/^\/D([0-9]+)/);
    if (m3)
        return phaseId === `D${m3[1]}`;
