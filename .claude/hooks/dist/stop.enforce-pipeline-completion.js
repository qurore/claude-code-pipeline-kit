...
import { clearAbortMarker, incrementStopInjections, isAborted, markStalled, } from './lib/pipeline-sentinel.js';
...
function resolveStateRoot(override) {
...
function appendLog(entry, stateRoot) {
    try {
...
export async function main(deps = {}) {
    ...
    const stderrWrite = deps.stderrWrite ?? ((s) => process.stderr.write(s));
    ...
    const reason = messages.stopBlockReason(m.pipeline, m.run_id, m.current_phase, m.iteration, m.max_iterations, next, max, nextCmd);
