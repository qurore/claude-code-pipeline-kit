...
import { archivableRuns, archiveRun, findInProgressRuns, isRecent, isStale, manifestToResumeEntry, } from './lib/state-discovery.js';
...
export async function main(env = process.env, now = Date.now()) {
    ...
    for (const run of toArchive) {
        try {
            ...
            emitSignal({ hook: HOOK, decision: 'warn', reason: 'archive-failed', context: { run: path.basename(run.runDir), error: err.message }, docsAnchor: 'session-resume' });
        }
    }
