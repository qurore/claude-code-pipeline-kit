...
import {
  ...
  getActivePipelines,
  startPipeline,
} from '../dist/lib/pipeline-sentinel.js';
...
function require(flags, names) {
  for (const n of names) {
    if (!(n in flags)) fail(`missing required flag --${n}`);
  }
}
...
try {
  switch (cmd) {
    ...
    case 'start': {
      ...
      if (f['current-phase']) opts.currentPhase = f['current-phase'];
      if (f['output-mode']) opts.outputMode = f['output-mode'];
      if (f['max-iterations']) opts.maxIterations = Number.parseInt(f['max-iterations'], 10);
    ...
    case 'complete': {
      ...
      if (summary === null) fail(`run-id not found: ${f['run-id']}`);
      // BR2 fix C1: snake_case JSON output.
      ...
      process.stderr.write(messages.pipelineCompleted(
        summary.pipeline,
        summary.runId,
        summary.totalPhases,
        summary.totalRestarts,
        summary.totalStopInjections,
        summary.durationMs,
      ) + '\n');
      break;
    }
