...
import type { HookHelp, HookInput } from './lib/types.js';

import { readFileSync } from 'node:fs';
import { resolve } from 'node:path';

function loadMigrationGlob(): string {
  ...
  try {
    ...
    const pkg = JSON.parse(readFileSync(pkgPath, 'utf-8')) as { claudeHooks?: { migrationGlob?: string } };
